...
function simulateNextFive(history) {
  let simHistory = [...history];
  const predictions = [];
  for (let i = 0; i < 5; i++) {
    const next = predictNext(simHistory);
    predictions.push(next);
    simHistory.push(next);
  }
  return predictions;
}

function generateRandomRounds(n = 10) {
  const symbols = ["莊", "閒", "和"];
  let result = "";
  for (let i = 0; i < n; i++) {
...
export default function BaccaratPredictor() {
  ...
  const validSymbols = ["莊", "閒", "和"];
  const cleanInput = input.replace(/\s|,|→/g, "");
  const clean = cleanInput.split("").filter(c => validSymbols.includes(c));
  const invalidChars = cleanInput.split("").filter(c => !validSymbols.includes(c));

  useEffect(() => {
    setHistory(clean);
    const next = predictNext(clean);
    setPrediction(next);
    setFuture(simulateNextFive(clean));
    if (clean.length >= 3) {
      setRecords(prev => [...prev, { input: clean.slice(-3).join(""), prediction: next }]);
    }
  }, [input]);
