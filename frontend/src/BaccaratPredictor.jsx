...
export default function BaccaratPredictor() { const [input, setInput] = useState(""); const [history, setHistory] = useState([]); const [prediction, setPrediction] = useState("");

const handlePredict = () => { const clean = input.replace(/\s|,|→/g, "").split("").filter(c => ["莊", "閒", "和"].includes(c)); setHistory(clean); const next = predictNext(clean); setPrediction(next); };

const handleClear = () => { setInput(""); setPrediction(""); setHistory([]); };

const handleAdd = (symbol) => { const updated = [...history, symbol]; setHistory(updated); setPrediction(predictNext(updated)); };

const chartData = history.map((item, index) => ({ index: index + 1, value: labelMap[item] }));
    ...
    <div className="mt-6">
      ...
      <ResponsiveContainer width="100%" height={200}>
        <LineChart data={chartData}>
          <XAxis dataKey="index" />
          <YAxis domain={[0, 2]} ticks={[0, 1, 2]} tickFormatter={(v) => reverseLabelMap[v]} />
          <Tooltip formatter={(v) => reverseLabelMap[v]} />
