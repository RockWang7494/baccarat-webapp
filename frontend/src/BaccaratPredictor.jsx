...
export default function BaccaratPredictor() {
  ...
  const handleAdd = async (symbol) => {
    ...
    const newLog = { round: updated.length, actual: symbol, predicted: next };
    setPredictionLog(prev => [...prev, newLog]);
  ...
  const chartData = useMemo(() =>
    history.map((label, index) => ({ name: index + 1, value: labelMap[label] }))
  , [history]);
    ...
    <div className="max-w-xl mx-auto mt-10 space-y-4">
      ...
      <input
        ...
        onChange={(e) => {
          ...
          setInput(value);
          const clean = value.replace(/\s|,|→/g, "").split("").filter(c => ["莊", "閒", "和"].includes(c));
          setHistory(clean);
