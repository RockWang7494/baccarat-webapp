import React, { useState } from "react"; import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, BarChart, Bar, Legend, CartesianGrid } from "recharts";
...
const patternAnalysis = () => { const result = { 莊: 0, 閒: 0 }; let last = null; let count = 0; history.forEach((val) => { if (val === last) { count++; } else { if (last === "莊" || last === "閒") { result[last] = Math.max(result[last], count); } count = 1; last = val; } }); if (last === "莊" || last === "閒") { result[last] = Math.max(result[last], count); } return [ { name: "莊連續次數", value: result["莊"] }, { name: "閒連續次數", value: result["閒"] } ]; };

const streakStats = () => { let longStreak = 0; let singleJumps = 0; let count = 1; for (let i = 1; i < history.length; i++) { if (history[i] === history[i - 1]) { count++; } else { if (count >= 3) longStreak++; if (count === 1) singleJumps++; count = 1; } } if (count >= 3) longStreak++; if (count === 1) singleJumps++; const total = longStreak + singleJumps; return { longStreak, singleJumps, longRate: total > 0 ? ((longStreak / total) * 100).toFixed(1) : 0, jumpRate: total > 0 ? ((singleJumps / total) * 100).toFixed(1) : 0 }; };

const stats = streakStats(); const drawChart = patternAnalysis();
