...
export default function BaccaratPredictor() {
  ...
  const handleAdd = (symbol) => {
    ...
    setPredictionLog((prev) => [
      ...prev,
      { round: updated.length, actual: symbol, predicted: next },
    ]);
  };

  const updateAccuracy = (records) => {
    const correct = records.filter((r) => r.actual === r.predicted).length;
    const total = records.length;
    setAccuracy(total > 0 ? ((correct / total) * 100).toFixed(1) : null);
  };
  ...
  const handleInputChange = (e) => {
    ...
    const clean = value
      .replace(/\s|,|→/g, "")
      .split("")
      .filter((c) => ["莊", "閒", "和"].includes(c));
    setHistory(clean);
    const next = predictNext(clean);
    setPrediction(next);
    updateAccuracy(records);
    simulateFuture(clean, simulationRounds);
  };

  const simulateFuture = (startHistory, rounds) => {
    let future = [];
    let current = [...startHistory];
    for (let i = 0; i < rounds; i++) {
      const next = predictNext(current);
      future.push(next);
      current.push(next);
    }
    setSimulatedResults(future);
  };
  ...
  const patternAnalysis = useMemo(() => {
    ...
    history.forEach((val) => {
      if (val === last) {
        ...
        if (last === "莊" || last === "閒") {
          result[last] = Math.max(result[last], count);
        }
        count = 1;
        last = val;
      }
    ...
    return [
      { name: "莊連續次數", value: result["莊"] },
      { name: "閒連續次數", value: result["閒"] },
    ];
  }, [history]);
  ...
  const streakStats = useMemo(() => {
    ...
    for (let i = 1; i < history.length; i++) {
      if (history[i] === history[i - 1]) {
        ...
        if (count >= 3) longStreak++;
        if (count === 1) singleJumps++;
        count = 1;
      }
    }
    if (count >= 3) longStreak++;
    if (count === 1) singleJumps++;
    const total = longStreak + singleJumps;
    return {
      longStreak,
      singleJumps,
      longRate: total > 0 ? ((longStreak / total) * 100).toFixed(1) : 0,
      jumpRate: total > 0 ? ((singleJumps / total) * 100).toFixed(1) : 0,
    };
  }, [history]);
  ...
  return (
    <div className="max-w-xl mx-auto mt-10 space-y-4">
      ...
      <div className="mt-4">
        ...
        <div className="text-sm mt-2">
          📈 長龍次數：{streakStats.longStreak}（{streakStats.longRate}%）｜單跳次數：
          {streakStats.singleJumps}（{streakStats.jumpRate}%）
        </div>
      </div>
      ...
      <div className="mt-4">
        ...
        <ul className="text-sm list-disc pl-5">
          {predictionLog.map((log, idx) => (
            <li key={idx}>
              第 {log.round} 局｜實際：{log.actual}｜預測：{log.predicted}
            </li>
          ))}
        </ul>
      </div>
